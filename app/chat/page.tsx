...
export default function ChatPage() {
  ...
  useEffect(() => {
    if (selectedUser && currentUser) {
      loadMessages()
      // Real-time message checking every 2 seconds
  ...
  const checkUserSession = async () => {
    ...
    if (token && user) {
      try {
        ...
        // Network error, keep user logged in for offline experience
        console.log("Session validation failed, keeping user logged in")
        setCurrentUser(JSON.parse(user))
      }
    } else {
